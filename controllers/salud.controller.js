...
const { saludModel } = require("../models");
const optionsPaginate = require("../config/paginationParams");



const addSalud = async (req, res) => {
  try {
    res.render('partials/salud/add')
  } catch (e) {
    handleHttpError(res, e);
  }
};

const editSalud = async (req, res) => {
  try {
    //req = matchedData(req); //revisar funcionamiento
    const { id } = req.params;
    const data = await saludModel.findById(id);
    res.render('partials/salud/edit', { data })
  } catch (e) {
    handleHttpError(res, e);
  }
};
...
const getSalud = async (req, res) => {
  console.log('muestyra todas las salud disponibles');
  try {
    const data = await saludModel.find({});
    console.log(data)
    res.render('partials/salud/list', { data })
  } catch (e) {
    handleHttpError(res, e);
  }
};
...
module.exports = { getSalud, addSalud, editSalud };